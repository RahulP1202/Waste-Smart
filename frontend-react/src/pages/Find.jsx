...
// -- Scrap Prices -------------------------------------------------------------
const SCRAP_PRICES = [
  { material:'Newspaper', material_kn:'???????', price:'Rs 12–15/kg', trend:'up', color:'#f59e0b', tip:'Bundle neatly for best price', tip_kn:'????? ?????? ?????????? ?????' },
  { material:'Cardboard', material_kn:'?????????????', price:'Rs 8–12/kg', trend:'stable', color:'#f59e0b', tip:'Flatten and keep dry', tip_kn:'?????? ???? ?????? ???' },
  { material:'White Paper', material_kn:'???? ????', price:'Rs 14–18/kg', trend:'up', color:'#f59e0b', tip:'Remove staples and clips', tip_kn:'??????? ????? ?????? ????????' },
  { material:'PET Bottles', material_kn:'PET ?????', price:'Rs 5–10/kg', trend:'stable', color:'#3b82f6', tip:'Crush and remove caps', tip_kn:'?????? ?????? ????????' },
  { material:'HDPE Plastic', material_kn:'HDPE ??????????', price:'Rs 8–12/kg', trend:'stable', color:'#3b82f6', tip:'Clean before selling', tip_kn:'????? ????? ????????' },
  { material:'Mixed Plastic', material_kn:'????? ??????????', price:'Rs 2–5/kg', trend:'down', color:'#3b82f6', tip:'Separate by type for better price', tip_kn:'????? ?????? ??? ?????? ?????????' },
  { material:'Aluminium Cans', material_kn:'??????????? ??????', price:'Rs 80–120/kg', trend:'up', color:'#6366f1', tip:'Crush to save space', tip_kn:'??? ?????? ??????' },
  { material:'Copper Wire', material_kn:'?????? ????', price:'Rs 400–500/kg', trend:'up', color:'#6366f1', tip:'Strip insulation for higher price', tip_kn:'??????? ?????? ?????????? ????????' },
  { material:'Steel/Iron', material_kn:'?????/??????', price:'Rs 20–30/kg', trend:'stable', color:'#6366f1', tip:'Remove non-metal parts', tip_kn:'?????? ????????? ????????' },
  { material:'Brass', material_kn:'????????', price:'Rs 280–320/kg', trend:'up', color:'#6366f1', tip:'Clean and separate grades', tip_kn:'???????????? ????? ?????? ?????????' },
  { material:'Glass Bottles', material_kn:'????? ?????', price:'Rs 2–5/piece', trend:'stable', color:'#10b981', tip:'Keep intact, no broken glass', tip_kn:'???????? ??????? ???' },
  { material:'Mobile Phones', material_kn:'?????? ????', price:'Rs 50–500/piece', trend:'up', color:'#ef4444', tip:'Factory reset before selling', tip_kn:'????? ????? ????????? ?????? ????' },
  { material:'Laptop/Computer', material_kn:'???????????/?????????', price:'Rs 500–3000/piece', trend:'up', color:'#ef4444', tip:'Remove personal data first', tip_kn:'????? ???????? ???? ?????' },
  { material:'Batteries', material_kn:'???????', price:'Rs 10–50/kg', trend:'stable', color:'#ef4444', tip:'Only at certified e-waste centers', tip_kn:'?????????? ?-??????? ??????????? ?????' },
  { material:'Cotton Cloth', material_kn:'????? ?????', price:'Rs 5–15/kg', trend:'stable', color:'#8b5cf6', tip:'Clean and dry before selling', tip_kn:'????? ????? ?????? ??????' },
  { material:'Rubber/Tyres', material_kn:'??????/????', price:'Rs 5–10/kg', trend:'stable', color:'#8b5cf6', tip:'Cut into pieces for better price', tip_kn:'????? ?????? ????? ????' },
]

// -- Marketplace constants -----------------------------------------------------
const MP_MATERIALS = ['All','Paper','Plastic','Metal','Glass','E-Waste','Cloth','Rubber','Cardboard','Copper','Aluminium','Steel']
const MP_MATERIALS_KN = { All:'?????', Paper:'????', Plastic:'??????????', Metal:'???', Glass:'????', 'E-Waste':'?-???????', Cloth:'?????', Rubber:'??????', Cardboard:'?????????????', Copper:'?????', Aluminium:'???????????', Steel:'?????' }
const KAB_MATERIALS_KN = { Paper:'????', Plastic:'??????????', Metal:'???', Glass:'????', 'E-Waste':'?-???????', Cloth:'?????', Cardboard:'?????????????', Batteries:'???????', Electronics:'???????????????', 'Mobile Phones':'?????? ????', Rubber:'??????', Copper:'?????', Aluminium:'???????????', Steel:'?????' }
  ...
  return (
    <div className="mp-card" onClick={() => onView(listing)}>
      <div className="mp-card-img-wrap">
        ...
        {listing.negotiable && <div className="mp-neg-badge">{lang==='kn'?'???????':'Negotiable'}</div>}
...
export default function Find({ session }) {
  ...
  function getLocation() {
    if (!navigator.geolocation) return setCMsg({ text: lang==='kn'?'GPS ?????????':'GPS not available on this device', ok:false })
    navigator.geolocation.getCurrentPosition(
      pos => {
        setCForm(f => ({ ...f, location_lat: pos.coords.latitude.toFixed(6), location_lng: pos.coords.longitude.toFixed(6) }))
        setCMsg({ text: lang==='kn'?'???? ?????????? ?':'Location captured ?', ok:true })
      },
      () => setCMsg({ text: lang==='kn'?'???? ?????? ??????????':'Could not get location. Please enter manually.', ok:false })
    )
  }
  ...
  async function openCamera() {
    ...
    } catch { setCMsg({ text: lang==='kn'?'???????? ??????? ????':'Could not open camera', ok:false }); setCamOpen(false) }
  }
  ...
  async function submitComplaint() {
    if (!cForm.title || !cForm.description || !cForm.location_text) return setCMsg({ text: lang==='kn'?'????????, ?????? ????? ???? ?????':'Title, description and location are required', ok:false })
    ...
    try {
      ...
      if (error) throw error
      setCMsg({ text: lang==='kn'?'???? ?????????? ????????????!':'Complaint submitted successfully!', ok:true })
      ...
      const errMsg = e.message === 'demo' || !supabaseConfigured
        ? (lang==='kn' ? 'Supabase ???????????????. ???? ?????? Supabase ????? ????.' : 'Supabase not connected. Please set up Supabase to save complaints.')
        : e.message
  ...
  const COMPLAINT_TYPES = {
    illegal_dumping: { en:'Illegal Dumping', kn:'????? ??????? ????', color:'#ef4444', icon:'???' },
    missed_collection: { en:'Missed Collection', kn:'?????? ???????', color:'#f59e0b', icon:'??' },
    open_burning: { en:'Open Burning', kn:'???? ???', color:'#f97316', icon:'??' },
    overflowing_bin: { en:'Overflowing Bin', kn:'????? ???????????? ????', color:'#8b5cf6', icon:'??' },
    water_pollution: { en:'Water Pollution', kn:'???? ???????', color:'#3b82f6', icon:'??' },
    other: { en:'Other', kn:'???', color:'#6b7280', icon:'??' },
  }
  const SEVERITY_COLORS = { low:'#10b981', medium:'#f59e0b', high:'#ef4444', critical:'#7c3aed' }
  const STATUS_LABELS = {
    pending: { en:'Pending', kn:'????', color:'#f59e0b' },
    under_review: { en:'Under Review', kn:'?????????????', color:'#3b82f6' },
    resolved: { en:'Resolved', kn:'?????????????', color:'#10b981' },
    rejected: { en:'Rejected', kn:'???????????????', color:'#ef4444' },
  }
    ...
    <div className="find-root">
      ...
      <div className="find-topbar">
        ...
        <div className="find-topbar-brand">
          <RecycleLogo size={22}/>
          <span>{lang === 'kn' ? '????? & ?????????' : 'Find & Sell Scrap'}</span>
        </div>
      ...
      <div className="find-hero">
        <h1>{lang === 'kn' ? '??????? ?????????? ??????' : 'Karnataka Scrap Hub'}</h1>
        <p>{lang === 'kn' ? '??????? ?????? ??????, ???? ???????? ????? ????? ?????????? ????' : 'Find recycling centers, check scrap prices, and sell your scrap'}</p>
      ...
      {activeTab === 'map' && (
        <div className="find-container">
          <div className="find-filters-row">
            ...
            <div className="find-result-count">{filteredKabs.length} {lang === 'kn' ? '?????????' : 'centers found'}</div>
          </div>
          ...
          <div className="find-map-wrap">
            ...
            <div className="find-map-overlay-note">
              ...
              {lang === 'kn' ? '?????? ????????? ?????? ????? ???? — ???????? 40+ ?????????' : 'Select a center below to get directions — 40+ centers across Karnataka'}
            </div>
          </div>
      ...
      {activeTab === 'prices' && (
        <div className="find-container">
          <div className="prices-header">
            <div>
              <h3>{lang === 'kn' ? '????? ?????????? ????' : "Today's Scrap Prices"}</h3>
              <p>{lang === 'kn' ? '???????? ????????? ?? — ??????? 2026' : 'Bengaluru market rates — April 2026'}</p>
            </div>
        ...
        <>
            ...
            <div className="mp-browse">
              <div className="mp-hero-inner">
                <div className="mp-hero-content">
                  <h2>{lang==='kn'?'????? ?????????? ????, ?? ?????':'Sell Your Scrap, Earn Money'}</h2>
                  <p>{lang==='kn'?'???????? ?????????? ????????? — ??? ???????????? ????':'Karnataka\'s scrap marketplace — sell directly to buyers and kabadiwalas'}</p>
                  <div className="mp-hero-stats">
                    <div><strong>{listings.length}</strong><small>{lang==='kn'?'????????':'Listings'}</small></div>
                    <div><strong>{new Set(listings.map(l=>l.location_area)).size}</strong><small>{lang==='kn'?'?????????':'Areas'}</small></div>
                    <div><strong>?{listings.reduce((a,l)=>a+(l.total_price||0),0).toLocaleString()}</strong><small>{lang==='kn'?'????? ?????':'Total Value'}</small></div>
                  </div>
                </div>
              </div>
              ...
              <div className="mp-filters-bar">
                ...
                <select value={areaFilter} onChange={e=>setAreaFilter(e.target.value)} className="mp-select">
                  {MP_AREAS.map(a=><option key={a} value={a}>{a==='All'?(lang==='kn'?'????? ?????????':'All Areas'):a}</option>)}
                </select>
                <select value={sortBy} onChange={e=>setSortBy(e.target.value)} className="mp-select">
                  <option value="newest">{lang==='kn'?'????? ?????':'Newest First'}</option>
                  <option value="price_low">{lang==='kn'?'????? ????':'Price: Low to High'}</option>
                  <option value="price_high">{lang==='kn'?'?????? ????':'Price: High to Low'}</option>
                </select>
                <span className="mp-count">{filteredListings.length} {lang==='kn'?'??????????':'results'}</span>
              </div>
          ...
          {mpView === 'sell' && (
            <div className="mp-form-container">
              <h2>{lang==='kn'?'??? ????? ??????':'Create New Listing'}</h2>
              {mpMsg && <div className={`mp-msg ${mpMsg.includes('success')?'success':'error'}`}>{mpMsg}</div>}
              <div className="mp-form">
                <div className="mp-field"><label>{lang==='kn'?'????????':'Title'} *</label><input placeholder={lang==='kn'?'???: ??? ????? ?????':'e.g. Old newspaper bundle'} value={form.title} onChange={e=>setForm({...form,title:e.target.value})}/></div>
                <div className="mp-field-row">
                  <div className="mp-field"><label>{lang==='kn'?'????? ???':'Material'}</label>
                  ...
                  <div className="mp-field"><label>{lang==='kn'?'??????':'Condition'}</label>
                    <select value={form.condition} onChange={e=>setForm({...form,condition:e.target.value})} className="mp-select-full">
                      <option value="excellent">{lang==='kn'?'?????????':'Excellent'}</option>
                      <option value="good">{lang==='kn'?'?????':'Good'}</option>
                      <option value="fair">{lang==='kn'?'??????':'Fair'}</option>
                    </select>
                  </div>
                </div>
                <div className="mp-field-row">
                  <div className="mp-field"><label>{lang==='kn'?'??? (kg)':'Weight (kg)'}</label><input type="number" placeholder="e.g. 25" value={form.quantity_kg} onChange={e=>setForm({...form,quantity_kg:e.target.value})}/></div>
                  <div className="mp-field"><label>{lang==='kn'?'???? (?/kg)':'Price (?/kg)'}</label><input type="number" placeholder="e.g. 12" value={form.price_per_kg} onChange={e=>setForm({...form,price_per_kg:e.target.value})}/></div>
                </div>
                ...
                <div className="mp-field"><label>{lang==='kn'?'??????':'Description'}</label><textarea placeholder={lang==='kn'?'???????? ????? ????...':'Describe your scrap...'} value={form.description} onChange={e=>setForm({...form,description:e.target.value})} rows={3}/></div>
                <div className="mp-field"><label>{lang==='kn'?'??????':'Area'}</label>
                ...
                <div className="mp-field-row">
                  <div className="mp-field"><label>{lang==='kn'?'???? ?????':'Phone'} *</label><input type="tel" placeholder="9XXXXXXXXX" value={form.contact_phone} onChange={e=>setForm({...form,contact_phone:e.target.value})}/></div>
                ...
                <label className="mp-negotiable-check">
                  ...
                  <span>{lang==='kn'?'???? ??????? ?????':'Price is negotiable'}</span>
                </label>
                <button className="mp-submit-btn" onClick={submitListing} disabled={submitting}>
                  {submitting?'Posting...':(lang==='kn'?'????? ????????':'Post Listing')}
                </button>
              </div>
            </div>
          )}
              ...
              <div className="mp-detail-body">
                <div className="mp-detail-top">
                  <div>
                    ...
                    <div className="mp-detail-price">?{selectedListing.total_price} {selectedListing.negotiable && <span className="mp-neg-tag">{lang==='kn'?'???????':'Negotiable'}</span>}</div>
              ...
              <div className="mp-contact-card">
                ...
                <div className="mp-contact-methods">
                  <a href={`tel:${selectedListing.contact_phone}`} className="mp-contact-method phone">
                    ...
                    <div><strong>{lang==='kn'?'??? ????':'Call Now'}</strong><span>{selectedListing.contact_phone}</span></div>
                  </a>
                  {selectedListing.contact_whatsapp && (
                    <a href={`https://wa.me/91${selectedListing.contact_whatsapp}?text=Hi, I'm interested in your listing: ${selectedListing.title} on Smart Waste`} target="_blank" rel="noopener noreferrer" className="mp-contact-method whatsapp">
                ...
                <div className="mp-contact-note">
                  ...
                  {lang==='kn'?'???? ????? ????? ???? ????? ???? ????????':'Confirm price and location before meeting. Stay safe.'}
                </div>
              </div>
            </div>
          )}
        ...
        <div className="cp-root">
          ...
          {cView === 'feed' && (
            <>
              <div className="cp-hero">
                ...
                <h2>{lang==='kn'?'???? ???????':'Complaint Portal'}</h2>
                <p>{lang==='kn'?'????? ??????? ????, ?????? ?????? ???? ??? ??????? ???????????? ???? ????? GPS ??????????? ???? ????':'Report illegal dumping, missed collection or other waste issues with photo and GPS location'}</p>
                <div className="cp-stats">
                  <div><strong>{complaints.length}</strong><small>{lang==='kn'?'????? ???????':'Total Reports'}</small></div>
                  <div><strong>{complaints.filter(c=>c.status==='resolved').length}</strong><small>{lang==='kn'?'?????????????':'Resolved'}</small></div>
                  <div><strong>{complaints.filter(c=>c.status==='pending'||c.status==='under_review').length}</strong><small>{lang==='kn'?'???? ???':'Pending'}</small></div>
                </div>
                <button className="cp-new-btn" onClick={() => setCView('new')}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                  {lang==='kn'?'??? ???? ???????':'File New Complaint'}
                </button>
              </div>

              <div className="cp-filters">
                <select value={cTypeFilter} onChange={e=>setCTypeFilter(e.target.value)} className="cp-select">
                  <option value="All">{lang==='kn'?'????? ??????':'All Types'}</option>
                ...
                <select value={cStatusFilter} onChange={e=>setCStatusFilter(e.target.value)} className="cp-select">
                  <option value="All">{lang==='kn'?'????? ??????':'All Status'}</option>
                  {Object.entries(STATUS_LABELS).map(([k,v])=><option key={k} value={k}>{lang==='kn'?v.kn:v.en}</option>)}
                </select>
                <span className="cp-count">{filteredComplaints.length} {lang==='kn'?'???????':'reports'}</span>
              </div>

              {cLoading ? (
                <div className="mp-loading"><div className="spinner"><div/><div/><div/></div><p>{lang==='kn'?'???? ?????????...':'Loading...'}</p></div>
              ) : (
                        ...
                        <div className="cp-card-body">
                          ...
                          <div className="cp-card-footer">
                            ...
                            <button className="cp-upvote" onClick={e=>{e.stopPropagation();upvoteComplaint(c)}}>
                              ...
                              {c.upvotes||0} {lang==='kn'?'?????':'Support'}
                            </button>
                          </div>
                          {c.severity === 'critical' && <div className="cp-critical-badge">{lang==='kn'?'?? ??????':'?? Critical'}</div>}
                        </div>
                      </div>
                    )
                  })}
                  {filteredComplaints.length === 0 && <div className="mp-empty"><h3>{lang==='kn'?'??????? ????':'No complaints found'}</h3><p>{lang==='kn'?'??????? ????????':'Try changing filters'}</p></div>}
                </div>
              )}
            </>
          )}
          ...
          {cView === 'new' && (
            <div className="cp-form-container">
              ...
              <h2>{lang==='kn'?'???? ???????':'File a Complaint'}</h2>
              <p className="cp-form-sub">{lang==='kn'?'???? ????? GPS ??????????? ???? ???? ???????. ?????? ???????????? ???????????????.':'Submit an accurate complaint with photo and GPS. Forwarded to local BBMP/municipal authorities.'}</p>
              ...
              {camOpen && (
                <div className="cp-camera-wrap">
                  ...
                  <div className="cp-camera-btns">
                    ...
                    <button className="cp-cancel-cam-btn" onClick={stopCamera}>{lang==='kn'?'?????':'Cancel'}</button>
                  </div>
                </div>
              )}
              ...
              <div className="cp-form">
                ...
                <div className="cp-photo-section">
                  <label className="cp-field-label">{lang==='kn'?'???? (????? ???????)':'Photo (Recommended)'}</label>
                  {cForm.photoPreview ? (
                ...
                <div className="mp-field-row">
                  <div className="mp-field">
                    <label className="cp-field-label">{lang==='kn'?'???? ???':'Complaint Type'} *</label>
                  ...
                  <div className="mp-field">
                    <label className="cp-field-label">{lang==='kn'?'???????':'Severity'}</label>
                    <select value={cForm.severity} onChange={e=>setCForm(f=>({...f,severity:e.target.value}))} className="mp-select-full">
                      <option value="low">{lang==='kn'?'?????':'Low'}</option>
                      <option value="medium">{lang==='kn'?'?????':'Medium'}</option>
                      <option value="high">{lang==='kn'?'??????':'High'}</option>
                      <option value="critical">{lang==='kn'?'??????':'Critical'}</option>
                    </select>
                  </div>
                </div>
                ...
                {/* Title */}
                <div className="mp-field">
                  <label className="cp-field-label">{lang==='kn'?'????????':'Title'} *</label>
                  <input placeholder={lang==='kn'?'???: ???? ??? ????? ??????? ????':'e.g. Illegal dumping near lake'} value={cForm.title} onChange={e=>setCForm(f=>({...f,title:e.target.value}))}/>
                </div>

                {/* Description */}
                <div className="mp-field">
                  <label className="cp-field-label">{lang==='kn'?'??????':'Description'} *</label>
                  <textarea rows={4} placeholder={lang==='kn'?'??????? ????? ???????? ???? — ????? ???????, ????? ???????, ??? ??????...':'Describe the issue in detail — how long, how much waste, what problem...'} value={cForm.description} onChange={e=>setCForm(f=>({...f,description:e.target.value}))}/>
                </div>

                {/* Location */}
                <div className="mp-field">
                  <label className="cp-field-label">{lang==='kn'?'????':'Location'} *</label>
                  <div className="cp-location-row">
                    <input placeholder={lang==='kn'?'???: 5?? ??????, ???????, ????????':'e.g. 5th Cross, Koramangala, Bengaluru'} value={cForm.location_text} onChange={e=>setCForm(f=>({...f,location_text:e.target.value}))} style={{flex:1}}/>
                ...
                {/* Contact */}
                <div className="mp-field">
                  <label className="cp-field-label">{lang==='kn'?'?????? ?????? (??????)':'Contact Number (Optional)'}</label>
                ...
                <button className="cp-submit-btn" onClick={submitComplaint} disabled={cSubmitting}>
                  {cSubmitting ? (lang==='kn'?'????????????????...':'Submitting...') : (
                    <><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>{lang==='kn'?'???? ???????':'Submit Complaint'}</>
                  )}
                </button>
                ...
                <div className="cp-authority-note">
                  ...
                  {lang==='kn'?'????? ???? BBMP / ?????? ?????? ???????????? ???????????????':'Your complaint will be forwarded to BBMP / local municipal authorities'}
                </div>
              </div>
            </div>
          )}
            ...
            <div className="cp-detail-container">
              ...
              <div className="cp-detail-body">
                ...
                <button className="cp-upvote-lg" onClick={()=>upvoteComplaint(selectedComplaint)}>
                  ...
                  {lang==='kn'?'????????':'Support this complaint'} ({selectedComplaint.upvotes||0})
                </button>
                <div className="cp-timeline">
                  <h4>{lang==='kn'?'?????? ?????????':'Status Timeline'}</h4>
