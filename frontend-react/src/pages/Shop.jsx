...
const CAT_LABELS_KN = { eco_products:'ಇಕೋ ಉತ್ಪನ್ನಗಳು', recycled_goods:'ಮರುಬಳಕೆ ಸರಕು', organic_food:'ಸಾವಯವ ಆಹಾರ', upcycled:'ಅಪ್‌ಸೈಕಲ್ಡ್', services:'ಸೇವೆಗಳು' }
...
export default function Shop({ session }) {
  ...
  async function claimReward(reward) {
    if (!userId) return setClaimMsg(lang==='kn'?'ಬಹುಮಾನ ಪಡೆಯಲು ಲಾಗಿನ್ ಮಾಡಿ':'Sign in to claim rewards')
    if (!supabaseConfigured) return setClaimMsg(lang==='kn'?'ಬಹುಮಾನ ಪಡೆಯಲು Supabase ಸಂಪರ್ಕಿಸಿ':'Connect Supabase to claim rewards')
    try {
      ...
      setClaimMsg(lang==='kn'?'ಬಹುಮಾನ ಪಡೆಯಲಾಗಿದೆ! ಕೂಪನ್ ಕೋಡ್‌ಗಾಗಿ ನಿಮ್ಮ ಪ್ರೊಫೈಲ್ ಪರಿಶೀಲಿಸಿ.':'Reward claimed! Check your profile for the coupon code.')
    } catch(e) { setClaimMsg(e.message.includes('unique') ? (lang==='kn'?'ಈ ಬಹುಮಾನ ಈಗಾಗಲೇ ಪಡೆದಿದ್ದೀರಿ!':'Already claimed this reward!') : e.message) }
  }
    ...
    <div className="shop-root">
      ...
      {view==='stores' && (
        <div className="shop-home">
            ...
            <div className="shop-cats">
              {['All',...Object.keys(CAT_LABELS)].map(c=>(
                <button key={c} className={'shop-cat-btn '+(catFilter===c?'active':'')} onClick={()=>setCatFilter(c)}>
                  {c==='All'?(lang==='kn'?'ಎಲ್ಲಾ':'All'):(CAT_ICONS[c]+' '+(lang==='kn'?CAT_LABELS_KN[c]:CAT_LABELS[c]))}
                </button>
              ))}
            </div>
          </div>
          ...
          {loading ? (
            <div className="shop-loading"><div className="spinner"><div/><div/><div/></div><p>{lang==='kn'?'ಅಂಗಡಿಗಳು ಲೋಡ್ ಆಗುತ್ತಿವೆ...':'Loading stores...'}</p></div>
              ...
              {filteredVendors.map(v => (
                <div key={v.id} className={'shop-store-card '+(v.is_featured?'featured':'')} onClick={()=>loadVendorProducts(v)}>
                  ...
                  <div className="shop-store-body">
                    ...
                    <div className="shop-store-cat">{CAT_ICONS[v.category]} {(lang==='kn'?CAT_LABELS_KN[v.category]:CAT_LABELS[v.category])||v.category}</div>
                ...
                {rewards.map(r => (
                  <div key={r.id} className="shop-reward-card">
                    ...
                    <div className="shop-reward-body">
                      <div className="shop-reward-type-badge">{r.reward_type==='lucky_draw'?`🎲 ${lang==='kn'?'ಲಕ್ಕಿ ಡ್ರಾ':'Lucky Draw'}`:r.reward_type==='free_product'?`🎁 ${lang==='kn'?'ಉಚಿತ ಉತ್ಪನ್ನ':'Free Product'}`:`🏷️ ${lang==='kn'?'ರಿಯಾಯಿತಿ':'Discount'}`}</div>
                      <h4>{r.title}</h4>
                      ...
                      <div className="shop-reward-footer">
                        ...
                        {r.valid_until && <span className="shop-reward-expiry">{lang==='kn'?'ಅವಧಿ ಮುಕ್ತಾಯ':'Expires'} {new Date(r.valid_until).toLocaleDateString('en-IN',{day:'numeric',month:'short'})}</span>}
