...
const TYPE_ICONS = { school:'ðŸ«', apartment:'ðŸ¢', office:'ðŸ’¼', general:'ðŸŒ¿' }
const TYPE_COLORS = { school:'#f59e0b', apartment:'#3b82f6', office:'#6366f1', general:'#10b981' }
const REACTIONS = ['ðŸ‘','ðŸŒ±','â™»ï¸','ðŸ”¥','ðŸ’š','ðŸŒ']
...
const DEMO_POSTS = [
  { id:'p1', community_id:'demo', user_name:'Priya S', user_color:'#10b981', content:'Just completed the 7-day no-plastic challenge! Switched to cloth bags and steel bottles ðŸŒ±', image_url:'https://images.unsplash.com/photo-1466611653911-95081537e5b7?w=600&q=80', post_type:'challenge', reactions:{'ðŸ‘':12,'ðŸŒ±':8,'ðŸ’š':5}, comment_count:2, created_at:new Date(Date.now()-2*3600000).toISOString() },
  { id:'p2', community_id:'demo', user_name:'Rahul M', user_color:'#6366f1', content:'Our apartment collected 45kg of dry waste this week! Heading to Hasiru Dala tomorrow. Who wants to join? â™»ï¸', image_url:null, post_type:'update', reactions:{'ðŸ‘':19,'â™»ï¸':11,'ðŸ”¥':6}, comment_count:3, created_at:new Date(Date.now()-5*3600000).toISOString() },
  { id:'p3', community_id:'demo', user_name:'Ananya K', user_color:'#f59e0b', content:'Scanned 10 items today â€” learned my old phone charger is e-waste needing special disposal ðŸ¤¯', image_url:'https://images.unsplash.com/photo-1607082348824-0a96f2a4b9da?w=600&q=80', post_type:'scan', reactions:{'ðŸ‘':7,'ðŸŒ':9,'ðŸŒ±':4}, comment_count:1, created_at:new Date(Date.now()-86400000).toISOString() },
]
const DEMO_COMMENTS = {
  p1:[{ id:'c1', user_name:'Kiran B', user_color:'#3b82f6', content:'Amazing! Day 3 and going strong ðŸ’ª', created_at:new Date(Date.now()-3600000).toISOString() },{ id:'c2', user_name:'Sunita R', user_color:'#8b5cf6', content:'What was the hardest part?', created_at:new Date(Date.now()-1800000).toISOString() }],
...
const DEMO_CHALLENGES = [
  { id:'ch1', title:'Scan 100 Items This Month', description:'Collectively scan 100 waste items using Tyajyadinda Tejassige AI', target_scans:100, current_scans:67, end_date:'2026-04-30', status:'active' },
...
const DEMO_COMMUNITY = { id:'demo', name:'Tyajyadinda Tejassige', description:'Official demo community', type:'general', join_code:'DEMO01', member_count:128, total_scans:1240, total_carbon_saved:89.4, avatar_color:'#10b981' }
...
export default function Community({ session }) {
  ...
  async function createCommunity() {
    ...
    if (isDemo) {
      ...
      setMsg('Community created! (Demo mode â€” connect Supabase to persist)')
      setView('home'); return
    }
    try {
  ...
  return (
    <div className="comm-root">
      <div className="comm-topbar">
        ...
        <div className="comm-topbar-brand"><RecycleLogo size={22}/><span>{lang==='kn' ? 'à²¸à²®à³à²¦à²¾à²¯' : 'Community'}</span></div>
        {view==='detail'
          ? <button className="comm-post-topbtn" onClick={()=>setShowPostBox(true)}>+ {lang==='kn' ? 'à²ªà³‹à²¸à³à²Ÿà³' : 'Post'}</button>
          : <div style={{width:80}}/>
        }
      </div>
      ...
      {view==='home' && (
        <div className="comm-home-root">
          ...
          {/* Cinematic image hero â€” completely different from landing */}
          <div className="comm-video-hero">
            <div className="comm-hero-bg"/>
            <div className="comm-hero-overlay"/>
            <div className="comm-hero-content">
              <div className="comm-hero-tag">{lang==='kn' ? 'à²•à²°à³à²¨à²¾à²Ÿà²• à²¸à²®à³à²¦à²¾à²¯' : 'Karnataka Community'}</div>
              <h1>{lang==='kn' ? 'à²’à²Ÿà³à²Ÿà²¾à²—à²¿ à²¬à²¦à²²à²¾à²µà²£à³† à²¤à²°à³‹à²£' : 'Change Starts Together'}</h1>
              <p>{lang==='kn' ? 'à²—à³à²‚à²ªà³ à²°à²šà²¿à²¸à²¿, à²¸à³à²ªà²°à³à²§à³†à²—à²³à²²à³à²²à²¿ à²­à²¾à²—à²µà²¹à²¿à²¸à²¿, à²¨à²¿à²®à³à²® à²ªà²°à²¿à²¸à²° à²ªà³à²°à²¯à²¾à²£ à²¹à²‚à²šà²¿à²•à³Šà²³à³à²³à²¿' : 'Join groups, take on challenges, share your eco journey and inspire your community'}</p>
              <div className="comm-hero-actions">
                <button className="comm-hero-btn-primary" onClick={()=>setView('create')}>
                  ...
                  {lang==='kn' ? 'à²—à³à²‚à²ªà³ à²°à²šà²¿à²¸à²¿' : 'Create Group'}
                </button>
                <button className="comm-hero-btn-secondary" onClick={()=>setView('join')}>
                  ...
                  {lang==='kn' ? 'à²—à³à²‚à²ªà²¿à²—à³† à²¸à³‡à²°à²¿' : 'Join a Group'}
                </button>
              </div>
            </div>
          ...
          <div className="comm-impact-strip">
            <div className="comm-impact-strip-item">
              <strong>{isDemo ? 'â€”' : stats.members.toLocaleString()}</strong>
              <span>{lang==='kn' ? 'à²¸à²•à³à²°à²¿à²¯ à²¸à²¦à²¸à³à²¯à²°à³' : 'Active Members'}</span>
            </div>
            <div className="comm-impact-strip-divider"/>
            <div className="comm-impact-strip-item">
              <strong>{isDemo ? 'â€”' : stats.scans.toLocaleString()}</strong>
              <span>{lang==='kn' ? 'à²¤à³à²¯à²¾à²œà³à²¯ à²¸à³à²•à³à²¯à²¾à²¨à³â€Œà²—à²³à³' : 'Waste Scans'}</span>
            </div>
            <div className="comm-impact-strip-divider"/>
            <div className="comm-impact-strip-item">
              <strong style={{color:'#10b981'}}>{isDemo ? 'â€”' : stats.carbon.toFixed(1)+' kg'}</strong>
              <span>COâ‚‚ {lang==='kn' ? 'à²‰à²³à²¿à²¤à²¾à²¯' : 'Saved'}</span>
            </div>
            <div className="comm-impact-strip-divider"/>
            <div className="comm-impact-strip-item">
              <strong>{isDemo ? 'â€”' : stats.groups.toLocaleString()}</strong>
              <span>{lang==='kn' ? 'à²—à³à²‚à²ªà³à²—à²³à³' : 'Groups'}</span>
            </div>
          </div>

          {/* How it works */}
          <div className="comm-how-section">
            <h2>{lang==='kn' ? 'à²‡à²¦à³ à²¹à³‡à²—à³† à²•à³†à²²à²¸ à²®à²¾à²¡à³à²¤à³à²¤à²¦à³†' : 'How It Works'}</h2>
            <div className="comm-how-grid">
              <div className="comm-how-card">
                ...
                <div className="comm-how-body">
                  ...
                  <h3>{lang==='kn' ? 'à²—à³à²‚à²ªà³ à²°à²šà²¿à²¸à²¿' : 'Create a Group'}</h3>
                  <p>{lang==='kn' ? 'à²¨à²¿à²®à³à²® à²…à²ªà²¾à²°à³à²Ÿà³â€Œà²®à³†à²‚à²Ÿà³, à²¶à²¾à²²à³† à²…à²¥à²µà²¾ à²•à²šà³‡à²°à²¿à²—à²¾à²—à²¿ à²—à³à²‚à²ªà³ à²°à²šà²¿à²¸à²¿' : 'Start a group for your apartment, school, or office and invite members with a code'}</p>
                </div>
              </div>
              <div className="comm-how-card">
                ...
                <div className="comm-how-body">
                  ...
                  <h3>{lang==='kn' ? 'à²¸à³à²ªà²°à³à²§à³†à²—à²³à²²à³à²²à²¿ à²­à²¾à²—à²µà²¹à²¿à²¸à²¿' : 'Take on Challenges'}</h3>
                  <p>{lang==='kn' ? 'à²¤à³à²¯à²¾à²œà³à²¯ à²¸à³à²•à³à²¯à²¾à²¨à³ à²®à²¾à²¡à²¿, à²…à²‚à²•à²—à²³à³ à²—à²³à²¿à²¸à²¿ à²®à²¤à³à²¤à³ à²²à³€à²¡à²°à³â€Œà²¬à³‹à²°à³à²¡à³â€Œà²¨à²²à³à²²à²¿ à²®à³‡à²²à³‡à²°à²¿' : 'Scan waste, earn points and climb the leaderboard as a team'}</p>
                </div>
              </div>
              <div className="comm-how-card">
                ...
                <div className="comm-how-body">
                  ...
                  <h3>{lang==='kn' ? 'à²…à²¨à³à²­à²µ à²¹à²‚à²šà²¿à²•à³Šà²³à³à²³à²¿' : 'Share Your Journey'}</h3>
                  <p>{lang==='kn' ? 'à²ªà³‹à²¸à³à²Ÿà³ à²®à²¾à²¡à²¿, à²ªà³à²°à²¤à²¿à²•à³à²°à²¿à²¯à²¿à²¸à²¿ à²®à²¤à³à²¤à³ à²‡à²¤à²°à²°à²¨à³à²¨à³ à²ªà³à²°à³‡à²°à³‡à²ªà²¿à²¸à²¿' : 'Post updates, react to others and inspire your community to do more'}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="comm-home-inner">
            ...
            {isDemo && (
              <div className="comm-demo-banner">
                <span>{lang==='kn' ? 'Demo Mode â€” Supabase à²¸à²‚à²ªà²°à³à²•à²¿à²¸à²¿' : 'Demo Mode â€” Connect Supabase for full features'}</span>
                <button onClick={()=>loadCommunityDetail(DEMO_COMMUNITY)}>{lang==='kn' ? 'à²¡à³†à²®à³Š à²¨à³‹à²¡à²¿' : 'View Demo Community'}</button>
              </div>
            )}
            ...
            {loading ? (
              ...
              <div className="comm-no-groups">
                ...
                <h3>{lang==='kn' ? 'à²‡à²¨à³à²¨à³‚ à²¯à²¾à²µà³à²¦à³‡ à²—à³à²‚à²ªà²¿à²²à³à²²' : 'No groups yet'}</h3>
                <p>{lang==='kn' ? 'à²®à³‡à²²à²¿à²¨ à²¬à²Ÿà²¨à³â€Œà²—à²³à²¨à³à²¨à³ à²¬à²³à²¸à²¿ à²—à³à²‚à²ªà³ à²°à²šà²¿à²¸à²¿ à²…à²¥à²µà²¾ à²¸à³‡à²°à²¿' : 'Use the buttons above to create or join a group'}</p>
              </div>
            ) : (
              <>
                {myCommunities.length > 0 && <h2 className="comm-section-title">{lang==='kn' ? 'à²¨à²¨à³à²¨ à²—à³à²‚à²ªà³à²—à²³à³' : 'My Groups'}</h2>}
                      ...
                      <div key={c.id} className="comm-group-card" onClick={()=>loadCommunityDetail(c)}>
                        ...
                        <div className="comm-group-card-body">
                          ...
                          <div className="comm-group-stats">
                            <div className="comm-gstat"><strong>{c.member_count||1}</strong><small>{lang==='kn'?'à²¸à²¦à²¸à³à²¯à²°à³':'Members'}</small></div>
                            <div className="comm-gstat"><strong>{c.total_scans||0}</strong><small>{lang==='kn'?'à²¸à³à²•à³à²¯à²¾à²¨à³':'Scans'}</small></div>
                            <div className="comm-gstat"><strong style={{color:'#10b981'}}>{(c.total_carbon_saved||0).toFixed(1)}kg</strong><small>COâ‚‚</small></div>
                          </div>
                          <div className="comm-group-code">{lang==='kn'?'à²•à³‹à²¡à³:':'Code:'} <strong>{c.join_code}</strong></div>
                        </div>
                      </div>
                    )
                  })}
      ...
      {view==='create' && (
        <div className="comm-container comm-form-container">
          <h2>{lang==='kn'?'à²¹à³Šà²¸ à²—à³à²‚à²ªà³ à²°à²šà²¿à²¸à²¿':'Create New Group'}</h2>
          {msg && <div className="comm-msg info">{msg}</div>}
          <div className="comm-form">
            <div className="comm-field"><label>{lang==='kn'?'à²—à³à²‚à²ªà²¿à²¨ à²¹à³†à²¸à²°à³':'Group Name'} *</label><input placeholder={lang==='kn'?'à²‰à²¦à²¾: à²•à³‹à²°à²®à²‚à²—à²² à²…à²ªà²¾à²°à³à²Ÿà³â€Œà²®à³†à²‚à²Ÿà³':'e.g. Koramangala Apartment'} value={newName} onChange={e=>setNewName(e.target.value)}/></div>
            <div className="comm-field"><label>{lang==='kn'?'à²µà²¿à²µà²°à²£à³†':'Description'}</label><textarea placeholder={lang==='kn'?'à²¨à²¿à²®à³à²® à²—à³à²‚à²ªà²¿à²¨ à²¬à²—à³à²—à³† à²¹à³‡à²³à²¿...':'Tell us about your group...'} value={newDesc} onChange={e=>setNewDesc(e.target.value)} rows={3}/></div>
            <div className="comm-field">
              <label>{lang==='kn'?'à²—à³à²‚à²ªà²¿à²¨ à²µà²¿à²§':'Group Type'}</label>
            ...
            <button className="comm-btn-primary" onClick={createCommunity} disabled={creating}>{creating?'Creating...':(lang==='kn'?'à²—à³à²‚à²ªà³ à²°à²šà²¿à²¸à²¿':'Create Group')}</button>
          </div>
        </div>
      )}
      ...
      {view==='join' && (
        <div className="comm-container comm-form-container">
          <h2>{lang==='kn'?'à²—à³à²‚à²ªà²¿à²—à³† à²¸à³‡à²°à²¿':'Join a Group'}</h2>
          <p className="comm-join-hint">{lang==='kn'?'6-à²…à²•à³à²·à²°à²¦ à²•à³‹à²¡à³ à²¨à²®à³‚à²¦à²¿à²¸à²¿':'Enter the 6-character join code from your group admin'}</p>
          {msg && <div className={'comm-msg '+(msg.includes('Joined')?'success':'info')}>{msg}</div>}
          <div className="comm-form">
            <div className="comm-field"><label>{lang==='kn'?'à²¸à³‡à²°à³à²ªà²¡à³† à²•à³‹à²¡à³':'Join Code'}</label><input placeholder="e.g. ABC123" value={joinCode} onChange={e=>setJoinCode(e.target.value.toUpperCase())} maxLength={6} style={{textTransform:'uppercase',letterSpacing:'6px',fontSize:'22px',textAlign:'center',fontWeight:800}}/></div>
            <button className="comm-btn-primary" onClick={joinCommunity} disabled={joining}>{joining?'Joining...':(lang==='kn'?'à²—à³à²‚à²ªà²¿à²—à³† à²¸à³‡à²°à²¿':'Join Group')}</button>
          </div>
        </div>
      )}
        ...
        <div className="comm-detail-root">
          <div className="comm-detail-banner" style={{background:'linear-gradient(135deg,'+selectedCommunity.avatar_color+'22,'+selectedCommunity.avatar_color+'08)'}}>
            <div className="comm-detail-banner-inner">
              ...
              <div className="comm-detail-info">
                ...
                <div className="comm-detail-code">
                  {lang==='kn'?'à²•à³‹à²¡à³:':'Code:'} <strong>{selectedCommunity.join_code}</strong>
            ...
            <div className="comm-impact-row">
              <div className="comm-impact-card"><strong style={{color:'#10b981'}}>{selectedCommunity.member_count||members.length}</strong><small>{lang==='kn'?'à²¸à²¦à²¸à³à²¯à²°à³':'Members'}</small></div>
              <div className="comm-impact-card"><strong style={{color:'#6366f1'}}>{selectedCommunity.total_scans||0}</strong><small>{lang==='kn'?'à²¸à³à²•à³à²¯à²¾à²¨à³':'Scans'}</small></div>
              <div className="comm-impact-card"><strong style={{color:'#f59e0b'}}>{(selectedCommunity.total_carbon_saved||0).toFixed(1)}kg</strong><small>COâ‚‚ {lang==='kn'?'à²‰à²³à²¿à²¤à²¾à²¯':'Saved'}</small></div>
              <div className="comm-impact-card"><strong style={{color:'#10b981'}}>{((selectedCommunity.total_carbon_saved||0)/21).toFixed(1)}</strong><small>{lang==='kn'?'à²®à²° à²¸à²®à²¾à²¨':'Trees Equiv.'}</small></div>
            </div>
          </div>
          ...
          <div className="comm-tabs-bar">
            {['feed','challenges','leaderboard'].map(t=>(
              <button key={t} className={'comm-tab '+(activeTab===t?'active':'')} onClick={()=>setActiveTab(t)}>
                {t==='feed'?(lang==='kn'?'ðŸ“° à²«à³€à²¡à³':'ðŸ“° Feed'):t==='challenges'?(lang==='kn'?'ðŸ† à²¸à³à²ªà²°à³à²§à³†à²—à²³à³':'ðŸ† Challenges'):(lang==='kn'?'ðŸ… à²²à³€à²¡à²°à³â€Œà²¬à³‹à²°à³à²¡à³':'ðŸ… Leaderboard')}
              </button>
            ))}
          </div>
            ...
            <div className="comm-feed-container">
              {showPostBox ? (
                  ...
                  <div className="comm-composer-top">
                    ...
                    <div className="comm-composer-right">
                      <div className="comm-post-types">
                        {[['update','ðŸ“¢',lang==='kn'?'à²…à²ªà³â€Œà²¡à³‡à²Ÿà³':'Update'],['challenge','ðŸ†',lang==='kn'?'à²¸à³à²ªà²°à³à²§à³†':'Challenge'],['scan','ðŸ”',lang==='kn'?'à²¸à³à²•à³à²¯à²¾à²¨à³':'Scan'],['tip','ðŸ’¡',lang==='kn'?'à²¸à²²à²¹à³†':'Tip']].map(([t,icon,label])=>(
                          <button key={t} className={'comm-ptype-btn '+(postType===t?'active':'')} onClick={()=>setPostType(t)}>{icon} {label}</button>
                        ))}
                      </div>
                      <textarea className="comm-composer-input" placeholder={lang==='kn'?'à²¨à²¿à²®à³à²® à²…à²¨à³à²­à²µ à²¹à²‚à²šà²¿à²•à³Šà²³à³à²³à²¿...':'Share your eco journey, challenge update, or tip...'} value={postContent} onChange={e=>setPostContent(e.target.value)} rows={3} autoFocus/>
                      {postImagePreview && (
                      ...
                      <div className="comm-composer-actions">
                        <label className="comm-img-btn">
                          ...
                          {lang==='kn'?'à²«à³‹à²Ÿà³‹':'Photo'}
                          <input ref={fileRef} type="file" accept="image/*" style={{display:'none'}} onChange={handlePostImage}/>
                        </label>
                        <div style={{display:'flex',gap:8,marginLeft:'auto'}}>
                          <button className="comm-cancel-btn" onClick={()=>{setShowPostBox(false);setPostContent('');setPostImagePreview(null)}}>{lang==='kn'?'à²°à²¦à³à²¦à³':'Cancel'}</button>
                          <button className="comm-submit-post-btn" onClick={submitPost} disabled={posting||!postContent.trim()}>{posting?'Posting...':(lang==='kn'?'à²ªà³à²°à²•à²Ÿà²¿à²¸à²¿':'Post')}</button>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <button className="comm-open-composer" onClick={()=>setShowPostBox(true)}>
                  ...
                  <span>{lang==='kn'?'à²¨à²¿à²®à³à²® à²…à²¨à³à²­à²µ à²¹à²‚à²šà²¿à²•à³Šà²³à³à²³à²¿...':'Share your eco journey...'}</span>
                </button>
              )}
              ...
              {posts.length===0 && (
                <div className="comm-no-posts">
                  ...
                  <h3>{lang==='kn'?'à²‡à²¨à³à²¨à³‚ à²¯à²¾à²µà³à²¦à³‡ à²ªà³‹à²¸à³à²Ÿà³ à²‡à²²à³à²²':'No posts yet'}</h3>
                  <p>{lang==='kn'?'à²®à³Šà²¦à²² à²ªà³‹à²¸à³à²Ÿà³ à²®à²¾à²¡à²¿!':'Be the first to post!'}</p>
                </div>
              )}

              {posts.map(post => {
                const ptColors = {update:'#6366f1',challenge:'#f59e0b',scan:'#10b981',tip:'#3b82f6'}
                const ptIcons = {update:'ðŸ“¢',challenge:'ðŸ†',scan:'ðŸ”',tip:'ðŸ’¡'}
                  ...
                  <div key={post.id} className="comm-post-card">
                    ...
                    <div className="comm-reactions-row">
                      ...
                      <button className="comm-comment-toggle" onClick={()=>loadComments(post.id)}>
                        ...
                        {post.comment_count||0} {lang==='kn'?'à²•à²¾à²®à³†à²‚à²Ÿà³':'comments'}
                      </button>
                    </div>
                    {openComments[post.id] && (
                      <div className="comm-comments-section">
                        ...
                        <div className="comm-add-comment">
                          ...
                          <input placeholder={lang==='kn'?'à²•à²¾à²®à³†à²‚à²Ÿà³ à²¸à³‡à²°à²¿à²¸à²¿...':'Add a comment...'} value={newComment[post.id]||''} onChange={e=>setNewComment(n=>({...n,[post.id]:e.target.value}))} onKeyDown={e=>e.key==='Enter'&&addComment(post.id)}/>
          ...
          {activeTab==='challenges' && (
            <div className="comm-tab-content">
              {challenges.length===0 && <div className="comm-no-posts"><div style={{fontSize:48}}>ðŸ†</div><h3>{lang==='kn'?'à²‡à²¨à³à²¨à³‚ à²¯à²¾à²µà³à²¦à³‡ à²¸à³à²ªà²°à³à²§à³† à²‡à²²à³à²²':'No challenges yet'}</h3></div>}
              {challenges.map(ch=>{
                ...
                return (
                  <div key={ch.id} className={'comm-challenge-card '+(done?'done':'')}>
                    ...
                    <h4>{ch.title}</h4>
                    <p>{ch.description}</p>
                    <div className="comm-challenge-progress-label"><span>{ch.current_scans||0} / {ch.target_scans} {lang==='kn'?'à²¸à³à²•à³à²¯à²¾à²¨à³':'scans'}</span><span style={{fontWeight:700,color:done?'#10b981':'#f59e0b'}}>{pct.toFixed(0)}%</span></div>
              ...
              <div className="comm-lb-podium">
                {members.slice(0,3).map((m,i)=>(
                  <div key={m.id} className={'comm-podium-item rank-'+(i+1)}>
                    <div className="comm-podium-crown">{i===0?'ðŸ‘‘':i===1?'ðŸ¥ˆ':'ðŸ¥‰'}</div>
              ...
              <div className="comm-leaderboard">
                {members.map((m,i)=>(
                  <div key={m.id} className={'comm-lb-row '+(m.user_id===userId?'mine':'')}>
                    <div className="comm-lb-rank" style={{color:i===0?'#f59e0b':i===1?'#94a3b8':i===2?'#cd7c2f':'var(--muted2)'}}>{i===0?'ðŸ¥‡':i===1?'ðŸ¥ˆ':i===2?'ðŸ¥‰':('#'+(i+1))}</div>
                ...
                {members.length===0 && <p className="comm-empty-text">{lang==='kn'?'à²‡à²¨à³à²¨à³‚ à²¯à²¾à²µà³à²¦à³‡ à²¸à²¦à²¸à³à²¯à²°à²¿à²²à³à²²':'No members yet'}</p>}
              </div>
            </div>
          )}

          <button className="comm-leave-btn" onClick={()=>leaveCommunity(selectedCommunity.id)}>{lang==='kn'?'à²—à³à²‚à²ªà³ à²¬à²¿à²¡à²¿':'Leave Group'}</button>
        </div>
      )}
    </div>
  )
